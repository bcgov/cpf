...
<html xml:lang="en">
  <head>
    <title><c:out value="${requestScope['jakarta.servlet.error.status_code']}" /> Error</title>
  ...
  <div class="container">
    <div class="panel panel-danger">
      <div class="panel-heading">
        <h3 class="panel-title"><c:out value="${requestScope['jakarta.servlet.error.status_code']}" /> Error</h3>
      </div>
      <div class="panel-body">
<c:if test="${not empty param['subTitle']}">
        <h4><c:out value="${param['subTitle']}" /></h4>
</c:if>
<c:if test="${not empty requestScope['jarkata.servlet.error.message']}">
        <pre><c:out value="${requestScope['jakarta.servlet.error.message']}" escapeXml="false" /></pre>
</c:if>
